...
class CalculadoraInterativa {
        ...
        let html = `
            <div class="resultado-visivel">
                <div class="resultado-header">
                    ...
                    ${
                        classificacao
                            ? `<span class="resultado-classificacao ${classificacao.classe}">${classificacao.texto}</span>`
                            : ""
                    }
                </div>
        `;
    ...
    atualizarHistoricoUI() {
        if (this.historico.length === 0) {
            this.historicoLista.innerHTML =
                '<p class="historico-vazio">Nenhum cálculo realizado ainda</p>';
            return;
        }
            ...
            html += `
                <div class="historico-item">
                    <div class="historico-info">
                        <div class="historico-tipo">${this.obterTituloCalculo(
                            item.tipo
                        )}</div>
    ...
    compartilharResultado() {
        const resultadoTexto =
            this.resultadosDiv.querySelector(".resultado-valor")?.textContent;
        const tipoCalculo = this.resultadosDiv.querySelector(
            ".resultado-header h3"
        )?.textContent;
            ...
            if (valor === "" || valor === null) {
                ...
                alert(
                    `Por favor, preencha o campo: ${
                        elemento.previousElementSibling?.textContent || id
                    }`
                );
                return false;
            }
...
function calcularDesconto() {
    ...
    const totalSemDesconto = precoOriginal * quantidade;
    const totalComDesconto = precoComDesconto * quantidade;
    const economiaTotal = totalSemDesconto - totalComDesconto;
    const dados = {
        tipo: "desconto",
        valor: `R$ ${precoComDesconto.toFixed(2)}`,
        classificacao: {
            texto: `${percentualDesconto}% OFF`,
            classe: "classificacao-normal",
        },
        detalhes: [
            {
                label: "Preço Original",
                valor: `R$ ${precoOriginal.toFixed(2)}`,
            },
            { label: "Desconto", valor: `R$ ${valorDesconto.toFixed(2)}` },
            {
                label: "Preço Final",
                valor: `R$ ${precoComDesconto.toFixed(2)}`,
            },
            ...
            },
            {
                ...
                valor: `R$ ${economiaTotal.toFixed(2)}`,
            },
        ],
        grafico: {
            tipo: "bar",
            label: "Comparação de Preços",
            labels: ["Sem Desconto", "Com Desconto", "Economia"],
            valores: [totalSemDesconto, totalComDesconto, economiaTotal],
            cores: [
                "rgba(220, 53, 69, 0.6)",
                "rgba(40, 167, 69, 0.6)",
                "rgba(102, 126, 234, 0.6)",
