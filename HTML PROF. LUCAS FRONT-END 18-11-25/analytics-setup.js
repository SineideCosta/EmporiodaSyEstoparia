...
class AnalyticsManager {
    constructor() {
        this.GA_MEASUREMENT_ID = "G-XNT1FCLTF6"; // SUBSTITUA pelo seu ID do GA4
        ...
        document.addEventListener("click", (e) => {
            ...
            if (target.classList.contains("btn-ler-mais")) {
                this.trackEvent("cta_click", {
                    ...
                    content_title:
                        target.closest("article")?.querySelector("h2, h3")
                            ?.textContent || "unknown",
                });
            }
            ...
            if (
                target.classList.contains("filtro-btn") ||
                target.classList.contains("filtro-blog")
            ) {
                this.trackEvent("filter_use", {
                    filter_type:
                        target.getAttribute("data-categoria") ||
                        target.getAttribute("data-filtro"),
                    page_type: window.location.pathname.includes("blog")
                        ? "blog"
                        : "gallery",
                });
            }
            ...
            if (target.tagName === "A") {
                ...
                if (
                    ...
                    this.trackEvent("file_download", {
                        ...
                        download_source:
                            target.closest("section")?.className || "unknown",
                    });
                }
            }
        });
    ...
    trackBusinessMetrics() {
        ...
        const productViews = document.querySelectorAll(
            ".produtos li, .artigo-card"
        );
                ...
                (entries) => {
                    entries.forEach((entry) => {
                        if (entry.isIntersecting) {
                            const productName =
                                entry.target.querySelector("h3, h2")
                                    ?.textContent || `produto_${index}`;
                            this.trackEvent("product_view", {
                                product_name: productName,
                                product_position: index + 1,
                                page_type: window.location.pathname.includes(
                                    "produtos"
                                )
                                    ? "products"
                                    : "blog",
...
function getAnalyticsReport() {
    return window.analyticsManager
        ? window.analyticsManager.getSessionReport()
        : null;
}
